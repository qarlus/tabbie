...
export function NewTabDashboard() {
  ...
  const [activityScope, setActivityScope] = useStoredState<string>("activity-scope", "all")
  ...
  const availableScopes = useMemo(() => new Set(["all", "personal", ...(data?.organizations.map((organization) => organization.login.toLowerCase()) ?? [])]), [data])
  useEffect(() => { if (data && !availableScopes.has(activityScope)) setActivityScope("all") }, [activityScope, availableScopes, data, setActivityScope])

  const matchesScope = (item: GithubItem) => {
    if (activityScope === "all") return true
    const owner = item.repository.split("/")[0]?.toLowerCase()
    return activityScope === "personal" ? owner === settings.githubUsername.toLowerCase() : owner === activityScope
  }
  const filteredData = {
    pulls: data?.pulls.filter(matchesScope) ?? [],
    issues: data?.issues.filter(matchesScope) ?? [],
    notifications: data?.notifications.filter(matchesScope) ?? [],
  }
  const stats = { pulls: filteredData.pulls.length, issues: filteredData.issues.length, notifications: filteredData.notifications.length }
  const scopeLabel = activityScope === "all" ? "All activity" : activityScope === "personal" ? "Personal" : data?.organizations.find((organization) => organization.login.toLowerCase() === activityScope)?.login ?? "Organization"
  const visibleRepos = activityScope === "all" ? repos : repos.filter((repo) => {
    try {
      const [owner] = new URL(repo.url).pathname.split("/").filter(Boolean)
      return owner?.toLowerCase() === (activityScope === "personal" ? settings.githubUsername.toLowerCase() : activityScope)
    } catch { return false }
  })
  ...
  return <main className="min-h-screen bg-background">
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1.65fr)_minmax(280px,0.75fr)]">
        <section className="flex min-w-0 flex-col gap-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div className="flex flex-col gap-2"><div><h2 className="text-base font-semibold">GitHub activity</h2><p className="text-xs text-muted-foreground">{scopeLabel} from the last 7 days</p></div>{settings.githubUsername && <label className="flex items-center gap-2 text-xs font-medium text-muted-foreground"><span>View</span><select className="h-9 min-w-44 rounded-lg border bg-background px-3 text-sm text-foreground" value={activityScope} onChange={(event) => setActivityScope(event.target.value)} aria-label="Filter GitHub activity"><option value="all">All activity</option><option value="personal">Personal</option>{data?.organizations.map((organization) => <option key={organization.login} value={organization.login.toLowerCase()}>{organization.login}</option>)}</select></label>}</div><Button variant="outline" size="sm" onClick={loadGithub} disabled={loading}><RefreshCw className={loading ? "animate-spin" : ""} data-icon="inline-start" />Refresh</Button></div>
